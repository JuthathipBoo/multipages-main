...
function Animation() {
  ...
  const handleKeyDown = (event) => {
    switch (event.code) {
      ...
      case "Digit1":
        setBallImage(`url("${ballbas}")`);
        break;

      case "Digit2":
        setBallImage(`url("${ballfoot}")`);
        break;

      case "Digit3":
        setBallImage(`url("${ballvol}")`);
        break;

      case "Digit4":
        setBallImage(`url("${ballhuman}")`);
        break;

      case "Digit5":
        setBallImage(`url("${ballcar}")`);
        break;

      case "Digit6":
        setBallImage(`url("${ballLogo}")`);
        break;

      case "Space":
        event.preventDefault(); // กันเลื่อนหน้าเว็บ
      ...
      <div className="animation">
        ...
        <div id="control">
          ...
          <button
            id="btnballLogo"
            className="btn btn-primary"
            onClick={() => handleImageChange('url("'+ballLogo+'")') }
          >
            Logo
          </button>
        </div>
